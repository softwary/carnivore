...
class GameLog extends StatefulWidget {
  ...
  const GameLog({Key? key, required this.gameId}) : super(key: key);
    ...
    _gameLogSubscription = _gameLogRef
      ...
      if (event.snapshot.value != null) {
        ...
        List<Map<String, dynamic>> newLogs = data.entries.map((entry) {
          Map<dynamic, dynamic> logData = entry.value as Map<dynamic, dynamic>; // Explicit cast
          return {
        ...
        newLogs.sort((b, a) => (a['timestamp'] as num).compareTo(b['timestamp'] as num)); // Cast to num
      ...
      children: [
        ...
        Expanded(
          child: _logs.isEmpty
              ? const Center(
                  child:
                      Text("No actions yet", style: TextStyle(color: Colors.white70)),
                )
              : ListView.builder(
...
Widget _buildLogMessage(Map<String, dynamic> log) {
    print("Log action: ${log['type']}, Player: ${log['playerId']}, Word: ${log['word']}, TileLetter: ${log['tileLetter']}, TileId: ${log['tileId']}, Status: ${log['status']}");
    ...
    switch (log['type']) {
      case 'flip_tile':
        message = "Player ${log['playerId'].toString().substring(0, 4)} flipped: ";
        if (log['tileLetter'] != null) {
          tileWidgets.add(
            TileWidget(
              ...
              isSelected: false,
              backgroundColor: tileBackgroundColor, // Use the variable here
            ),
          );
        }
        break;
      ...
      case 'MIDDLE_WORD':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted: ";
          ...
          for (var letter in log['word'].split('')) {
            tileWidgets.add(
              TileWidget(
                ...
                onClickTile: (_, __, ___) {},
                isSelected: false,
                backgroundColor: tileBackgroundColor,  // Use the variable
              ),
            );
          }
        }
      ...
      case 'OWN_WORD_IMPROVEMENT':
        ...
        break;
      case 'STEAL_WORD':
        message = "Player ${log['playerId'].toString().substring(0, 4)} stole! ";
        textColor = Colors.green; // Valid word
        tileBackgroundColor = Colors.purple[900]!; // Valid word background
        if (log['word'] != null) {
          for (var letter in log['word'].split('')) {
      ...
      case 'INVALID_LENGTH':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted a word without enough letters: ";
      ...
      case 'INVALID_NO_MIDDLE':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted a word without using tiles from the middle: ";
        textColor = Colors.red;
        tileBackgroundColor = Colors.red[900]!;  // Invalid word background
      ...
      case 'INVALID_LETTERS_USED':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted a word without valid letters: ";
        textColor = Colors.red;
        tileBackgroundColor = Colors.red[900]!;  // Invalid word background
      ...
      case 'INVALID_WORD_NOT_IN_DICTIONARY':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted a word not in the dictionary! ";
        textColor = Colors.red;
        tileBackgroundColor = Colors.red[900]!;  // Invalid word background
      ...
      case 'INVALID_UNKNOWN_WHY':
        message = "Player ${log['playerId'].toString().substring(0, 4)} submitted an invalid word: ";
        textColor = Colors.red;
        tileBackgroundColor = Colors.red[900]!;  // Invalid word background
        ...
        break;
      default:
        message = "Unknown action by Player ${log['playerId'].toString().substring(0, 4)}";
    }
